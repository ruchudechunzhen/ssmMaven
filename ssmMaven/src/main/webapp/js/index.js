...
// 获取登录<a>标签的id,并给它绑定一个点击事件
$('#login_open').click(function () {
	// 获取登陆弹出框的<div> id 并将其class中的hidden(隐藏)删除
	// 所以点击登录，会出现一个登录弹框
...
function commitLogin(){
	alert("进来了")
   // 获取id名为loginEmail的标签的value值
   var email =$("#loginEmail").val();
   // 获取id名为loginPassword的标签的value值
   var password =$("#loginPassword").val();
   if(null!=email && email!="" && null!=password && password!=""){
	   // 将id名为loginForm的form表单中的值序列化
        var params=$("#loginForm").serialize();
       // alert(params);
        // post要小写
        // 将params提交给user/loginUser.do,提交之后执行回调函数function(data)
